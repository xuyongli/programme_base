...
function isValidSudoku(board: char[][]): boolean {
    ...
    for(let i = 0; i < 9; i++) {
        for (let j = 0; j < 9; j++) {
            const ch = board[i][j];
            if (ch !== '.') {
                const num = ch.charCodeAt(0) - '0'.charCodeAt(0) - 1;
                rows[i][num]++;
                cols[j][num]++;
                subboxes[Math.floor(i / 3)][Math.floor(j / 3)][num]++;
                if (rows[i][num] > 1 || cols[j][num] > 1 || subboxes[Math.floor(i / 3)][Math.floor(j / 3)][num] > 1) {
                    return false;
                }
            }
        }
