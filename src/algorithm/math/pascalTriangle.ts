// 杨辉三角
function pascalTriangle(lineNumber: number): number[] {
  const currentLine = [1]
...
function pascalTriangleRecursive(lineNumber: number): number[] {
  if (lineNumber === 0) {
    return [1]
  }
...
console.log(pascalTriangleRecursive(4).toString())
